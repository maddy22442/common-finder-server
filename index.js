...
// Use multer's memory storage to handle file uploads in serverless environments
const storage = multer.memoryStorage();
const upload = multer({ storage: storage });

// Function to process uploaded files from memory
function formatFile(buffer, originalname) {
  try {
    let content = buffer.toString('utf-8').trim().replace(/\r\n/g, '\n');

    if (originalname.endsWith('.json') || originalname.endsWith('.json.txt')) {
      try {
        const jsonData = JSON.parse(content);
        if (!Array.isArray(jsonData)) {
          console.error(`Invalid JSON array in file: ${originalname}`);
          return null;
        }
        content = jsonData.join('\n') + '\n';
      } catch (e) {
        console.error(`Error parsing JSON in file ${originalname}:`, e.message);
        return null;
      }
    }

    if (!content.length) {
      console.error(`File ${originalname} is empty or invalid.`);
      return null;
    }

    return content.split('\n').map(addr => addr.trim()).filter(addr => addr.length > 0);
  } catch (error) {
    console.error(`Error formatting file ${originalname}:`, error.message);
    return null;
  }
}

// Find common addresses from multiple files
function findCommonAddresses(filesData) {
  if (filesData.length === 0) return new Set();

  let commonAddresses = new Set(filesData[0]);

  for (let i = 1; i < filesData.length; i++) {
    commonAddresses = new Set([...commonAddresses].filter(addr => filesData[i].includes(addr)));
    if (commonAddresses.size === 0) break;
  }
...
app.post('/api/find-common', upload.array('files', 10), (req, res) => {
  try {
    ...
    const formattedFilesData = req.files.map(file => formatFile(file.buffer, file.originalname)).filter(Boolean);

    if (formattedFilesData.length < 2) {
      return res.status(400).json({ error: 'Invalid or improperly formatted files' });
    }

    const commonAddresses = findCommonAddresses(formattedFilesData);
